...
/**
 * Creates an order, customer (if not exists), and order_items in Supabase.
 */
export async function createOrder(
  ...
  try {
    ...
    const customerId = await findOrCreateCustomer(formData);

    // 2. Generate unique order_id in ORD-yymmdd-101 format
    ...
    if (itemsError) {
      ...
      // We still have the order, but return notice
    }
